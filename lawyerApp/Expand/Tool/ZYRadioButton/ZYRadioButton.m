...
-(void)defaultInit{
    ...
    _button.frame = CGRectMake(0, 10,kRadioButtonWidth, kRadioButtonHeight);
    ...
    UITapGestureRecognizer *singleTap=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(handleButtonTap:)];
    
    singleTap.numberOfTapsRequired=1;//轻击一次
...
+(void)buttonSelected:(ZYRadioButton*)radioButton{
    ...
    if (rb_instances) {
        for (int i = 0; i < [rb_instances count]; i++) {
            ...
            if ([button isEqual:radioButton]==NO&&[radioButton.groupId isEqualToString:button.groupId]==YES) {
